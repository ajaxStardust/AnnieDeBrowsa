...
• Proven Checks are generic. No project-specific CI or test runner
  is configured at time of first stewardship pass (2026-04-30-INITIAL-SETUP).
...
<!-- INVARIANT: This table reflects the Annie DeBrowsa SPA preview tool project.
     Update this table when project structure changes. -->

| Area | Canonical File(s) | Role / System Interaction |
|---|---|---|
| **Root Entry Point** | `public/default.php` | Bootstrap for path-to-URL transformation app. |
| **Main View** | `src/View/Main.page.php` | Renders transformation UI with Vue.js integration. |
| **Path Normalization Layer** | `src/Model/PathTransformer.php` | Normalizes filesystem paths, removes protocols, extracts components. |
| **URL Construction Layer** | `src/Model/UrlBuilder.php` | Constructs URLs from normalized components with host/protocol. |
| **URL Evaluation Layer** | `src/Model/UrlEvaluator.php` | Validates URLs, applies host-specific filtering rules. |
| **Path Processing Form** | `src/View/Trypath.form.php` | Generates three URL variations using $pipelineData from new pipeline. |
| **Legacy Path Normalization** | `src/Model/P2u2.php` | Deprecated tangled normalization logic (preserved for reference). |
| **Legacy URL Construction** | `src/Model/Newmethod.php` | Deprecated tangled URL construction logic (preserved for reference). |
| **Legacy Path Evaluation** | `src/Model/Evalpath.php` | Deprecated tangled evaluation logic (preserved for reference). |
| **Environment Display** | `src/Model/PathNormalizer.php` | Normalizes display paths for environment summary. |

...
### 4. Path-to-URL Transformation Pipeline (Annie DeBrowsa Project)
- **Rule**: The path-to-URL transformation MUST follow strict separation of concerns: Normalization → Construction → Evaluation.
- **Invariant**: `src/Model/PathTransformer` (Normalization) MUST NOT construct URLs or apply host-specific logic.
- **Invariant**: `src/Model/UrlBuilder` (Construction) MUST NOT normalize paths or validate URLs.
- **Invariant**: `src/Model/UrlEvaluator` (Evaluation) MUST NOT normalize paths or construct URLs.
- **Invariant**: `src/View/Main.page.php` MUST use the new separated pipeline classes.
- **Invariant**: `src/View/Trypath.form.php` MUST use `$pipelineData` structure from the new pipeline.
- **POSTCONDITION**: Each layer MUST return structured arrays with exact keys for downstream consumption.

...
## Required Reading Order (Trivium + standby)
...
4. [FUTURE.md](FUTURE.md): planned intent and roadmap (non-binding until promoted into law or run steps).
...
- Plan-only or prospective work change => update [FUTURE.md](FUTURE.md)
...
3. [src/View/Main.page.php](src/View/Main.page.php) bootstraps the environment, requires [src/View/Trypath.form.php](src/View/Trypath.form.php) for path-to-URL conversion UI, mounts the Vue result card, and loads the supporting scripts
...
### Path-to-URL Result Scheme Rule

- Converted **clickable** URLs from the path-to-URL tool are **always** built with the **`http://`** scheme.
- Implementation lives in [src/View/Trypath.form.php](src/View/Trypath.form.php) (live page), [src/View/Vue-path2url.php](src/View/Vue-path2url.php) (alternate fragment), [public/content/form-trypath.content.php](public/content/form-trypath.content.php) (public-composition variant), and the debug link HTML in [src/Model/Newmethod.php](src/Model/Newmethod.php) (`buildUrlLast`).
- There is **no** `REQUEST_SCHEME`, TLS detection, env flag, or config-driven scheme for these outputs: this is a **developer convenience** tool; HTTPS upgrades and redirects are the target host’s or ops concern.
- Do not reintroduce scheme-selection logic without an explicit product decision and a matching update to this contract.
...
12. The policy that path-to-URL **output** links always use `http://` changes, or the set of files that emit those links changes.
13. The primary UX/styling entrypoints for the live default / shell experience change (see Governance Update Log for the current list).
...
Within an active session, a **coding agent** (governance steward: GitHub Copilot, Cursor, or similar) is expected to update the **narrowest** owning artifact when contract-designated triggers are encountered, per [WHY.md](WHY.md).

When the repository owner grants **steward authority**, it applies **only** to `./.contract/**/*.md` (create, modify, delete, or append) unless they explicitly widen scope in writing.

Before a session ends after scope-affecting work, the steward **reconciles `./.contract/`** so logs, invariants, and `LAST REVIEWED` dates match reality.
...
- Stewardship language broadened from Copilot-only to any in-session coding agent acting as governance steward.
